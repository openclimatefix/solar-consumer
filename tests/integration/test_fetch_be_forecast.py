...
def test_be_generation_contains_national_and_regional():
    """
    Integration test for Belgium solar generation data fetching (realtime).

    Ensures that the Elia API integration returns both:
    - national (Belgium)
    - at least one regional generation data
    """

    df = fetch_be_data_generation()
    ...
    assert "Belgium" in regions, "National Belgium generation data missing"
