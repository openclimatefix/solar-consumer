    ...
    data = pd.DataFrame(
        {
            ...
            "target_datetime_utc": pd.to_datetime(
                ["2025-01-14 05:30:00", "2025-01-14 06:00:00"] * 3
            ),
        }
    )
...
def test_make_potential_generation(mock_entsoe_pandas_client):
    ...
    assert (
        potential_generation.iloc[48].solar_generation_no_curtailment_kw == 1.11
    )  # 12:00 has been increased
    ...
    assert (
        potential_generation.loc[negative_prices_idx, "solar_generation_no_curtailment_kw"] == 1.11
    ).all()
    assert (
