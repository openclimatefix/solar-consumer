...
# Unit tests for generation data fetching

@freeze_time("2026-01-18T10:00:00Z")
def test_fetch_be_generation_mixed_regions(requests_mock):
    """
    Validate successful generation ingestion with:
    - mocked Elia API URL (generation endpoint)
    - national + regional records
    - MW -> kW conversion (using realtime field)
    - cursor-based pagination termination
    """
    ...
    requests_mock.get(
        ...
        [
            {
                "text": load_mock_response("elia_be_generation_mixed_regions.json"),
                "status_code": 200,
            },
            {
                "json": {"results": []},
    ...
    with patch(
        ...
        df = fetch_be_data_generation(days=1)
    ...
    # MW -> kW conversion (realtime field)
    assert df.loc[df["region"] == "Belgium", "solar_generation_kw"].iloc[0] == 2500
    assert df.loc[df["region"] == "Flanders", "solar_generation_kw"].iloc[0] == 800
...
def test_fetch_be_generation_empty_response(requests_mock):
    """
    Ensure an empty API response results in:
    - an empty DataFrame
    - correct output schema for generation
    """
...
def test_fetch_be_generation_invalid_payload(requests_mock):
    """
    Ensure unexpected / malformed payloads do not crash the fetcher
    and result in an empty DataFrame (generation).
    """
